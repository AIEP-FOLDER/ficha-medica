...
function checkRut(rut) {
    ...
    if(cuerpo.length < 7) { 
        ...
        document.getElementById('err-rut').textContent = "RUT Incompleto";
        return false;
    }
    ...
    if(dvEsperado != dv) { 
        ...
        document.getElementById('err-rut').textContent = "RUT Inválido. Ej: 12.345.678-5";
        return false; 
    }
    
    // Si todo sale bien, eliminar errores (decretar que es válido)
    rut.setCustomValidity('');
    document.getElementById('err-rut').textContent = '';
}
...
async function loadPacientes(apellido = '') {
  try {
    const url = apellido 
      ? `http://localhost:3000/api/pacientes/buscar?apellido=${encodeURIComponent(apellido)}`
      : 'http://localhost:3000/api/pacientes';
    
    const response = await fetch(url);
    if (response.ok) {
      const pacientes = await response.json();
      renderResultados(pacientes);
    } else {
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(buscar) buscar.addEventListener('input',e=>loadPacientes(e.target.value));
  loadPacientes('');
  ...
  btnGuardar?.addEventListener('click',async ()=>{
    ...
    try {
      ...
      if (response.ok) {
        msg.textContent = result.message || 'Registro guardado correctamente.';
        msg.classList.add('ok');
        ...
        msg.textContent = result.error || 'Error al guardar el registro.';
        msg.classList.add('error');
        msg.classList.remove('ok');
      }
    } catch (error) {
      console.error('Error:', error);
      msg.textContent = 'Error de conexión. Verifique que el servidor esté ejecutándose.';
      msg.classList.add('error');
      msg.classList.remove('ok');
    }
  });
